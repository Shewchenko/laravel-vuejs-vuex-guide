...
/** Чтобы код был наглядным, я задублировал код с shops/actions */
...
export default {
    [actionTypes.A_GET_LIST]: (context, payload) => {
        api.post('/api/users/all').then(res => {
    ...
    [actionTypes.A_ADD]: (context, payload) => {
        ...
        api.post('/api/users/save', payload).then(res => {
            if (res.status === 200) {
    ...
    [actionTypes.A_DELETE]: (context, payload) => {
        // api.post('/api/users/delete', {id:payload.id}).then(res => {
        //     if (res.status === 200) {
